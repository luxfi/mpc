...
func (ec *eventConsumer) handleSigningEventSR25519(msg *types.SignTxMessage, natMsg *nats.Msg) {
	...
	// Create SR25519 signing session
	// Use "substrate" as default signing context
	...
	if err != nil {
		...
		logger.Error("Failed to create SR25519 signing session", err)
		ec.handleSigningSessionError(
			msg.WalletID,
			msg.TxID,
			msg.NetworkInternalCode,
	...
	// Wait for completion
	go func() {
		logger.Info("Waiting for SR25519 signing session to finish", "sessionID", msg.TxID, "walletID", msg.WalletID)
		result := session.WaitForFinish()
		logger.Info("SR25519 signing session WaitForFinish returned", "sessionID", msg.TxID, "result", result)
		done()
		ec.sendReplyToRemoveMsg(natMsg)
		logger.Info("SR25519 signing session completed", "result", result)
	}()
}
