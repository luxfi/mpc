...
func TestLiveness_RejectsBadSig(t *testing.T) {
	pub, priv, _ := ed25519.GenerateKey(rand.Reader)
	att := LivenessAttestation{ProviderID: "securegate", UserID: "u-1", Score: 0.9, Timestamp: time.Now().Unix()}
	env := signEnvelope(t, priv, att)
...
func TestLiveness_RejectsWrongUser(t *testing.T) {
	pub, priv, _ := ed25519.GenerateKey(rand.Reader)
	att := LivenessAttestation{ProviderID: "securegate", UserID: "attacker", Score: 0.9, Timestamp: time.Now().Unix()}
	env := signEnvelope(t, priv, att)
...
func TestLiveness_RejectsStale(t *testing.T) {
	pub, priv, _ := ed25519.GenerateKey(rand.Reader)
	att := LivenessAttestation{ProviderID: "securegate", UserID: "u-1", Score: 0.9, Timestamp: time.Now().Add(-10 * time.Minute).Unix()}
	env := signEnvelope(t, priv, att)
	_, err := VerifyLiveness(env, &LivenessOpts{PubKey: pub, UserID: "u-1", MinScore: 0.8, MaxAge: time.Minute})
...
func TestLiveness_RejectsLowScore(t *testing.T) {
	pub, priv, _ := ed25519.GenerateKey(rand.Reader)
	att := LivenessAttestation{ProviderID: "securegate", UserID: "u-1", Score: 0.5, Timestamp: time.Now().Unix()}
	env := signEnvelope(t, priv, att)
	_, err := VerifyLiveness(env, &LivenessOpts{PubKey: pub, UserID: "u-1", MinScore: 0.8, MaxAge: time.Minute})
...
func TestLiveness_RejectsWrongProvider(t *testing.T) {
	...
	env := signEnvelope(t, priv, att)
	_, err := VerifyLiveness(env, &LivenessOpts{PubKey: pub, ProviderID: "securegate", UserID: "u-1", MinScore: 0.8, MaxAge: time.Minute})
...
// observing a fresh challenge could persuade SecureGate to sign an
...
// extended envelope post-SecureGate-rollout).
...
// This is the "SecureGate not yet rolled out challengeId" case. The
...
func TestLiveness_R38_LaxAcceptsCredentialHashOnly(t *testing.T) {
	...
	att := LivenessAttestation{
		...
		Timestamp:      time.Now().Unix(),
		CredentialHash: "HASH-OK",
		// no ChallengeID — simulates the current SecureGate envelope
	}
	env := signEnvelope(t, priv, att)
	_, err := VerifyLiveness(env, &LivenessOpts{
		PubKey: pub, UserID: "u-1", MinScore: 0.8, MaxAge: time.Minute,
