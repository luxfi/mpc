...
const (
	...
	// window while SecureGate rolls out the extended envelope. Tracked
	...
	if requiresBinding {
		...
		if !matched {
			if opts.BindingMode == BindingStrict {
				return nil, errors.New("liveness: envelope carries no binding (credentialHash/challengeId); SecureGate must emit the extended envelope")
			}
			if opts.WarnFn != nil {
				opts.WarnFn("liveness: envelope carries no binding — accepted in LAX mode; tracked SecureGate rollout")
			}
		}
	}
	return &att, nil
